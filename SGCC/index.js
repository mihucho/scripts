...
// 初始化存储
wsgwStore = new Store(NAMESPACE);

// 网络请求实现
const wsgwRequest = async (request) => {
  try {
...
const Recoginze = async e => {
  ...
  const response = await wsgwRequest(o);
  return JSON.parse(response.body);
};

// 主要的网络请求函数
const wsgwApiRequest = async e => {
  try {
    const o = {
...
// 网上国网数据获取功能
WsgwGlobal.bizrt = jsonParse(wsgwStore.get("95598_bizrt")) || {};
...
async function getKeyCode() {
  ...
  try {
    const e = {
      url: `/api${$api.getKeyCode}`,
      method: "post",
      headers: {}
    };
    WsgwGlobal.requestKey = await wsgwApiRequest(e);
    console.log("✅ 获取keyCode和publicKey成功");
  } catch (e) {
...
// 获取验证码
async function getVerifyCode(username, password) {
  console.log("⏳ 获取验证码...");
  try {
    const e = {
...
// 登录功能
async function login(e, o, username, password) {
  console.log("⏳ 登录中...");
  try {
    const r = {
  ...
  try {
    const e = {
      ...
      headers: {
        ...WsgwGlobal.requestKey,
        token: WsgwGlobal.bizrt.token,
        authorizecode: WsgwGlobal.authorizecode
      }
    };
    WsgwGlobal.accessToken = await wsgwApiRequest(e).then((e => e.access_token));
    console.log("✅ 获取凭证成功");
  } catch (e) {
  ...
  try {
    const e = {
      ...
      headers: {
        ...WsgwGlobal.requestKey,
        token: WsgwGlobal.bizrt.token,
        acctoken: WsgwGlobal.accessToken
      },
      data: {
        ...
        quInfo: {
          userId: WsgwGlobal.bizrt.userInfo[0].userId
        },
        token: WsgwGlobal.bizrt.token,
        Channels: "web"
      }
    };
    WsgwGlobal.bindInfo = await wsgwApiRequest(e).then((e => e.bizrt));
    console.log("✅ 获取绑定信息成功");
  } catch (e) {
...
async function getElcFee(e) {
  console.log("⏳ 查询电费...");
  try {
    const o = WsgwGlobal.bindInfo.powerUserList[e];
    const [r] = WsgwGlobal.bizrt.userInfo;
    const s = {
      url: `/api${$api.accapi}`,
      method: "post",
      headers: {
    ...
    WsgwGlobal.eleBill = await wsgwApiRequest(s).then((e => e.list[0]));
    console.log("✅ 查询电费成功");
  } catch (e) {
...
async function getDayElecQuantity(e) {
  ...
  try {
    ...
    const c = await wsgwApiRequest(t);
    console.log("✅ 获取日用电量成功");
    WsgwGlobal.dayElecQuantity = c;
  } catch (e) {
...
async function getMonthElecQuantity(e) {
  ...
  try {
    ...
    const s = await wsgwApiRequest(e);
    console.log("✅ 获取月用电量成功");
    WsgwGlobal.monthElecQuantity = s;
  } catch (e) {
...
// 网上国网数据获取完整流程
async function fetchWsgwData(username, password) {
